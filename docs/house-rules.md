...
* Random encounters are checked once for every started 4-hour watch block and camping activities don't increase the DC. If watch is disabled, rest and random encounter checks use an 8-hour duration: if you don't check for random encounters, many camping activities are obsolete and there's no tension
...
Furthermore, giving your players ways to bypass direct exploration will remove a lot of the fun of exploring the Stolen Lands, so I'd advise against that.
